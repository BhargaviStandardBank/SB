...
Object.defineProperty(voltmx, 'application', {configurable:false, enumerable:false, writable:false, value:(function() {
    ...
    var _setApplicationCallbacks = function $KAPP_setApplicationCallbacks(callbacksObj) {
        var $K = voltmx.$kwebfw$, $KU = $K.utils;
        $KU.log({ api: 'voltmx.application.setApplicationCallbacks', enter: true });

        // Error 101: Invalid number of arguments
        if (arguments.length !== 1) {
            throw new Error("101 - Invalid number of arguments: Expected exactly one argument.");
        }

        // Error 100: Invalid type of parameters
        if (typeof callbacksObj !== 'object' || callbacksObj === null || Array.isArray(callbacksObj)) {
            throw new Error("100 - Invalid type of parameters: Expected a non-null object.");
        }

        // Error 102: Invalid input structure
        const validKeys = ['onactive', 'oninactive', 'onbackground', 'onforeground', 'onappterminate'];
        const invalidKeys = Object.keys(callbacksObj).filter(key => !validKeys.includes(key) || typeof callbacksObj[key] !== 'function');
        if (invalidKeys.length > 0) {
            throw new Error("102 - Invalid input error: Unexpected or invalid callback(s): " + invalidKeys.join(', '));
        }

        // onactive
        if (callbacksObj.onactive) {
            window.addEventListener('focus', () => {
                callbacksObj.onactive();
            });
        }

        // oninactive
        if (callbacksObj.oninactive) {
            window.addEventListener('blur', () => {
                callbacksObj.oninactive();
            });
        }

        // onbackground / onforeground
        if (callbacksObj.onbackground || callbacksObj.onforeground) {
            document.addEventListener('visibilitychange', () => {
                if (document.visibilityState === 'hidden' && callbacksObj.onbackground) {
                    callbacksObj.onbackground();
                }
                if (document.visibilityState === 'visible' && callbacksObj.onforeground) {
                    callbacksObj.onforeground();
                }
            });
        }

        // onappterminate
        if (callbacksObj.onappterminate) {
            window.addEventListener('beforeunload', () => {
                callbacksObj.onappterminate();
            });
        }

        $KU.log({ api: 'voltmx.application.setApplicationCallbacks', exit: true });
    };
    ...
    $K.defVoltmxProp(_ns, [
        ...
        {keey:'setApplicationCallbacks', value:_setApplicationCallbacks},
...
Object.defineProperty(voltmx, 'i18n', {configurable:false, enumerable:false, writable:false, value:(function() {
    ...
    var _getBrowserLanguage = function $KI18N_getBrowserLanguage() {
        ...
        // Platform-specific fix: apply only for DESKTOPNATIVE builds
...
/* FILE PATH :: 'lib/apis/voltmximage.js' */
...
Object.defineProperty(voltmx, 'os', {configurable:false, enumerable:false, writable:false, value:(function() {
    ...
    var _hasPasskeySupport = async function() {
        var $K = voltmx.$kwebfw$, $KU = $K.utils, flag = false;
        // Check WebAuthn API support in the browser
        if (window.PublicKeyCredential) {
            try {
                const available = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
                $KU.log({api:'voltmx.os.hasPasskeySupport', enter:true});
                if (available) {
                    $KU.log('Passkeys are supported on this device.');
                    return true;
                } else {
                    $KU.log('Passkeys are not supported on this device.');
                    return false;
                }
            } catch (err) {
                $KU.log({api:'voltmx.os.hasPasskeySupport', enter:true});
                $KU.log('Error checking WebAuthn support:', err);
                return false;
            }
        } else {
            $KU.log({api:'voltmx.os.hasPasskeySupport', enter:true});
    ...
    $K.defVoltmxProp(_ns, [
        ...
        {keey:'hasPasskeySupport', value:_hasPasskeySupport},
