...
$KI.os = {
    ...
    hasPasskeySupport: async function() {
        
        if (window.PublicKeyCredential) {
            try {
                const available = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
                $KU.logExecuting('voltmx.os.hasPasskeySupport');
                if (available) {
                    $KU.logExecuting('Passkeys are supported on this device.');
                    return true;
                } else {
                    $KU.logExecuting('Passkeys are not supported on this device.');
                    return false;
                }
            } catch (err) {
                $KU.logExecuting('voltmx.os.hasPasskeySupport');
                $KU.logExecuting('Error checking WebAuthn support:', err);
                return false;
            }
        } else {
            $KU.logExecuting('voltmx.os.hasPasskeySupport');
